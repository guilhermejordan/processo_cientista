...
1. As variáveis `raintoday` e `raintomorrow` estão codificadas de forma diferente, isso deve ser tratado na fase de modelagem.
...
Uma breve análise desta variável sugere que se trata das predições dadas por um modelo adotado atualmente para a mesma tarefa. Isso sugere um objetivo para este projeto: **obter um modelo melhor que o modelo vigente**. Para isso, vamos avaliar o modelo vigente e compará-lo ao modelo que vamos construir.
...
Notamos que as bases `wind_table_01` e `wind_table_01` usam nomes de colunas diferente das demais. Vamos corrigir isso e consolidar todas as bases numa única tabela:
...
A partir de agora, vamos utilizar alguns pacotes que facilitam a automatização da análise exploratória [@staniak2019landscape]. Vamos focar em pacotes que geram relatórios automaticamente. O script que gera esses relatórios é o `exploratory_analysis/generate_reports.R`.

## Diagnóstico da base

O relatório pode ser visualizado no arquivo [reports/dlookr_diagnose.html](/processo_cientista/exploratory_analysis/reports/dlookr_diagnose.html).

### Destaques:

1. As variáveis `evaporation` e `sunshine` têm mais de 98 % de observações missing, sendo de pouca utilidade para a modelagem.
2. As variáveis `cloud9am` e `cloud3pm` também tem muitas observações missing (> 37 %), requerendo atenção na fase de modelagem.
